...
// The UsageRequestCaptureFunc type is an adapter to allow the use of ordinary
// function as UsageRequestCapture mutator.
type UsageRequestCaptureFunc func(context.Context, *ent.UsageRequestCaptureMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f UsageRequestCaptureFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.UsageRequestCaptureMutation); ok {
		return f(ctx, mv)
	}
