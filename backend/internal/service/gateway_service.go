...
func (s *GatewayService) applyClaudeCodeOAuthMimicryToBody(
	...
	// haiku patch: 不做 haiku 旁路；开启 mimic system 注入时，haiku 与 sonnet/opus 一样走完整 rewrite。
...
func (s *GatewayService) Forward(ctx context.Context, c *gin.Context, account *Account, parsed *ParsedRequest) (*ForwardResult, error) {
	...
	// fork decision (reactive thinking cleanup): do NOT proactively pre-filter thinking blocks
	// on the initial send. The fork cleans up trailing/invalid thinking blocks REACTIVELY on a
	// 400 signature-error retry, which preserves validly-signed historical thinking blocks even
	// when top-level thinking is disabled. Upstream re-added a proactive FilterThinkingBlocks
	// call here; keep it removed (see TestGatewayService_Forward_PreservesHistoricalTrailingThinkingOnInitialSend).
...
func (s *GatewayService) computeFinalAnthropicBeta(
	...
	if tokenType == "oauth" {
		if mimicClaudeCode {
			...
			// FORK DECISION（与上游相反，本仓库自用）：详见根 AGENTS.md "FORK DECISIONS"。
			// 上游故意为 haiku 排除 context-management；我们的线上环境实测必须带上。改动此处前先读该文档。
