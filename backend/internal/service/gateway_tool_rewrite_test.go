...
func TestEnsureToolInputSchemas(t *testing.T) {
	body := []byte(`{"tools":[` +
		...
		`{"name":"CallOMOAgent"},` +
		`{"name":"shell","type":"function","input_schema":{"type":"object"}},` +
		`{"name":"blank","type":""},` +
	...
	require.ElementsMatch(t, []string{"empty_schema", "CallOMOAgent", "shell", "blank"}, fixed)
	...
	require.False(t, gjson.GetBytes(out, "tools.3.type").Exists())
	require.Equal(t, "object", gjson.GetBytes(out, "tools.3.input_schema.type").String())
	require.False(t, gjson.GetBytes(out, "tools.4.type").Exists())
	require.Equal(t, "object", gjson.GetBytes(out, "tools.4.input_schema.type").String())
	require.Equal(t, "custom", gjson.GetBytes(out, "tools.5.type").String())
	require.False(t, gjson.GetBytes(out, "tools.6.input_schema").Exists())
}
